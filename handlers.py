...
async def debug_sales_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin-only diagnostic command: shows the sales config exactly as the
    bot sees it right now (which backend, which fields are set vs empty,
    and whether each empty field's SALES_* environment variable is present
    in this process). Created specifically to make "the VIP message shows
    no price/banks" diagnosable from Telegram itself, without guessing."""
    from bot import ADMIN_USER_ID
    ...
    config = SalesConfigManager()
    backend = "Upstash Redis" if config.use_upstash else f"archivo local ({config.file_path})"

    fields = [
        ("Precio VIP", config.get_vip_price(), "SALES_VIP_PRICE"),
        ("Banco Guayaquil", config.get_bank_guayaquil_details(), "SALES_BANK_GUAYAQUIL"),
        ("Banco Pichincha", config.get_bank_pichincha_details(), "SALES_BANK_PICHINCHA"),
        ("PayPal", config.get_paypal_details(), "SALES_PAYPAL"),
        ("Enlace grupo demo", config.get_demo_group_link(), "SALES_DEMO_GROUP_LINK"),
        ("Enlace grupo VIP", config.get_vip_group_link(), "SALES_VIP_GROUP_LINK"),
        ("FAQ", config.get_faq_text(), "SALES_FAQ_TEXT"),
    ]

    lines = [f"🔧 *DEBUG VENTAS*\n\nBackend de almacenamiento: {backend}\n"]
    for label, value, env_name in fields:
        ...
        if not is_default:
            status = f"✅ Configurado: {value[:60]}"
        elif env_value:
            status = f"⚠️ Variable `{env_name}` está definida pero NO se está usando (ya hay un valor guardado con prioridad)"
        else:
            status = f"❌ Vacío — variable `{env_name}` no está definida en este proceso"
...
def register_ventas_handlers(application):
    ...
    application.add_handler(CommandHandler("debug_sales", debug_sales_command))
