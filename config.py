...
def _default_config() -> dict:
    ...
    Precedence, from lowest to highest: hardcoded fallback here < Railway
    environment variable (SALES_*, if set) < a value already saved in
    Upstash/local file (see _merge_with_defaults - stored data always wins
    once anything has been saved).

    This means: on a fresh deploy, before anything has ever been saved,
    setting these SALES_* environment variables in Railway is enough to
    configure the whole sales system with no code edits and no manual
    Upstash writes. Once a value is saved for the first time (e.g. by a
    future admin-panel config flow), that stored value takes over on every
    subsequent load, and further changes to the Railway variable stop
    having an effect - the same "seed once, then persist" precedence
    already used for PROMOTION_INTERVAL/BotState elsewhere in this project.
    """
    return {
        "vip_price": os.getenv("SALES_VIP_PRICE", "Consultar precio"),
        "bank_guayaquil_details": os.getenv("SALES_BANK_GUAYAQUIL", ""),
        "bank_pichincha_details": os.getenv("SALES_BANK_PICHINCHA", ""),
        "paypal_details": os.getenv("SALES_PAYPAL", ""),
        "demo_group_link": os.getenv("SALES_DEMO_GROUP_LINK", ""),
        "vip_group_link": os.getenv("SALES_VIP_GROUP_LINK", ""),
        "faq_text": os.getenv(
            "SALES_FAQ_TEXT", "Aún no se ha configurado el texto de preguntas frecuentes."
        ),
    }
...
class SalesConfigManager:
    ...
    def _log_loaded_config(self) -> None:
        """Diagnostic log fired every time the sales config loads, so a
        misconfiguration (e.g. SALES_* env vars not actually reaching the
        process) is immediately visible in Railway's log stream instead of
        only showing up as "missing data" inside a Telegram message with no
        clue why. For each field, logs whether it currently holds a real
        value or is empty/default."""
        backend = "Upstash Redis" if self.use_upstash else f"local file ({self.file_path})"
        fields = [
            ("vip_price", "SALES_VIP_PRICE"),
            ("bank_guayaquil_details", "SALES_BANK_GUAYAQUIL"),
            ("bank_pichincha_details", "SALES_BANK_PICHINCHA"),
            ("paypal_details", "SALES_PAYPAL"),
            ("demo_group_link", "SALES_DEMO_GROUP_LINK"),
            ("vip_group_link", "SALES_VIP_GROUP_LINK"),
            ("faq_text", "SALES_FAQ_TEXT"),
        ]
        logger.info(f"[ventas.config] Loaded sales config (backend: {backend}):")
        for field, env_name in fields:
            value = self.data.get(field, "")
            has_value = bool(value) and value not in (
                "Consultar precio",
                "Aún no se ha configurado el texto de preguntas frecuentes.",
            )
            env_is_set = bool(os.getenv(env_name, "").strip())
            status = "OK (con valor)" if has_value else (
                f"VACÍO/DEFAULT - variable de entorno {env_name} "
                f"{'está definida pero el valor guardado la está ignorando' if env_is_set else 'NO está definida en este proceso'}"
            )
            logger.info(f"[ventas.config]   {field}: {status}")
